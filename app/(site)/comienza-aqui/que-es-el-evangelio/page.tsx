...
export default async function WhatIsGospelPage() {
  ...
  const supportingTitle = locale === 'pt' ? 'Aprofunde' : 'Profundiza'
  const supportingSubtitle =
    locale === 'pt'
      ? 'Um vídeo e um resumo para ver como o evangelho se encaixa na história da Bíblia.'
      : 'Un video y un resumen para ver cómo el evangelio encaja en la historia de la Biblia.'
    ...
    <div className="min-h-screen">
      ...
      <section className="border-t border-gray-200 bg-gray-50/60 py-16">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="mb-10">
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">{supportingTitle}</h2>
            <p className="mt-2 text-gray-600 max-w-2xl">{supportingSubtitle}</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-lg shadow-lg p-8 border-t-4 border-[#1E88E5] hover:shadow-xl transition-shadow">
              ...
              <p className="text-gray-600 leading-relaxed mb-4">{dict.gospel.problem.description}</p>
              <p className="text-sm text-gray-500 italic">{dict.gospel.problem.verse}</p>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-8 border-t-4 border-[#FBC02D] hover:shadow-xl transition-shadow">
              ...
              <p className="text-gray-600 leading-relaxed mb-4">{dict.gospel.solution.description}</p>
              <p className="text-sm text-gray-500 italic">{dict.gospel.solution.verse}</p>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-8 border-t-4 border-[#43A047] hover:shadow-xl transition-shadow">
              ...
              <p className="text-gray-600 leading-relaxed mb-4">{dict.gospel.response.description}</p>
              <p className="text-sm text-gray-500 italic">{dict.gospel.response.verse}</p>
            </div>
          </div>
        </div>
      </section>
